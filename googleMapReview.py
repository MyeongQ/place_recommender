...
number = 5  # 1~14
batch_size = 10
places = pd.read_csv("Busan_popular_places.csv", header="infer")
...
number = str(number)
df.to_csv("Busan_utility_matrix"+number+".csv", index=False)
#df.to_csv("Busan_utility_matrix_"+'1_2.csv', index=False)
err_df = pd.DataFrame(np.array(error))
err_df.to_csv("Busan_utility_error"+number+".csv", index=False)
